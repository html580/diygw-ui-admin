		...
		<el-dialog title="操作日志详细" v-model="state.open" width="700px" center append-to-body>
			<el-form ref="ruleFormRef" :model="state.modelForm" label-width="100px">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="请求地址：">{{ state.modelForm.operUrl }} </el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="操作方法：">{{ state.modelForm.method }} </el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="请求参数：">{{ state.modelForm.operParam }} </el-form-item>
					</el-col>
					<el-col :span="12">
...
import { clearData, delData, listData } from '@/api';
...
const handleClean = () => {
	ElMessageBox({
		...
		.then(function () {
			return clearData('/log/operations');
		})
		.then(() => {
			handleQuery();
...
const onTabelRowDel = (row: any) => {
	...
	ElMessageBox({
		...
		return delData('/log/operations', operIds).then(() => {
			handleQuery();
