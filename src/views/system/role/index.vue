	...
	<div class="container">
		<el-card shadow="always">
			...
			<div class="mb8">
				...
				<el-button type="danger" plain v-auth="'system:role:delete'" :disabled="state.multiple"
					@click="handleDelete">
					<SvgIcon name="ele-Delete" />删除
				</el-button>
				<el-button type="warning" plain v-auth="'system:role:export'" @click="handleExport">
			...
			<el-table v-loading="state.loading" border :data="state.roleList" @selection-change="handleSelectionChange">
				...
				<el-table-column label="操作" align="center" fixed="right" width="280">
					<template #default="scope">
						<el-button type="text" v-auth="'system:role:edit'" @click="handleUpdate(scope.row)">
							<SvgIcon name="ele-Edit" />修改
						</el-button>
						<el-button type="text" @click="handleDataScope(scope.row)">
							<SvgIcon name="ele-CircleCheck" />数据权限
						</el-button>
						<el-button type="text" v-auth="'system:role:delete'" @click="handleDelete(scope.row)">
							<SvgIcon name="ele-Delete" />删除
						</el-button>
					</template>
				</el-table-column>
		...
		<el-dialog :title="state.title" v-model="state.open" width="769px" center>
			<el-form ref="roleFormRef" :model="state.roleForm" :rules="state.rules" label-width="120px"
					...
					<el-col :xs="24">
						<el-form-item label="权限设置">
							<div>
								...
								<el-tree class="tree-border" :data="state.menuOptions" show-checkbox ref="menuRef"
									node-key="menuId" empty-text="加载中，请稍后" :props="{
										label: 'menuName',
										children: 'children',
				...
				<el-row>
					...
					<el-col :xs="24">
						<el-form-item label="数据权限" v-show="state.roleForm.dataScope == 2">
							<div>
								<el-checkbox v-model="state.deptExpand"
									@change="handleCheckedTreeExpand($event, 'dept')">展开/折叠 </el-checkbox>
								<el-checkbox v-model="state.deptNodeAll"
									@change="handleCheckedTreeNodeAll($event, 'dept')">全选/全不选 </el-checkbox>
								<el-tree class="tree-border" :data="state.deptOptions" show-checkbox default-expand-all
									ref="deptRef" node-key="deptId" empty-text="加载中，请稍后" :props="{
										label: 'deptName',
										children: 'children',
									}"></el-tree>
							</div>
