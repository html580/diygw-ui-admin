		...
		<el-card shadow="always">
			...
			<el-form :model="state.queryParams" ref="queryForm" :inline="true" label-width="68px">
				<el-form-item label="参数名称" prop="configName">
					<el-input placeholder="参数名称模糊查询" clearable @keyup.enter="handleQuery" style="width: 240px"
						v-model="state.queryParams.configName" />
				</el-form-item>
				<el-form-item label="参数键名" prop="configKey">
					<el-input placeholder="参数键名模糊查询" clearable @keyup.enter="handleQuery" style="width: 240px"
			...
			<el-row :gutter="10" class="mb8">
				<el-col :span="1.5">
					<el-button type="primary" plain @click="onOpenAddModule" v-auth="'system:config:add'">
						<SvgIcon name="ele-Plus" />新增
					</el-button>
				</el-col>
				<el-col :span="1.5">
					<el-button type="danger" plain :disabled="state.multiple" v-auth="'system:config:delete'"
						@click="onTabelRowDel">
						<SvgIcon name="ele-Delete" />删除
					</el-button>
				</el-col>
				<el-col :span="1.5">
					<el-button type="warning" plain v-auth="'system:config:export'" @click="handleExport">
			...
			<el-table v-loading="state.loading" border :data="state.tableData" @selection-change="handleSelectionChange">
				...
				<el-table-column label="系统内置" align="center" prop="configType">
					<template #default="scope">
						<el-tag :type="scope.row.configType === '1' ? 'danger' : 'success'" disable-transitions>{{
				...
				<el-table-column label="操作" align="center" fixed="right" width="180">
					<template #default="scope">
						<el-button type="text" v-auth="'system:config:edit'" @click="onOpenEditModule(scope.row)">
							<SvgIcon name="ele-Edit" />修改
						</el-button>
						<el-button v-if="scope.row.parentId != 0" type="text" v-auth="'system:config:delete'"
...
const handleExport = () => {
	...
	ElMessageBox({
		...
		.then(function () {
			return exportData('/sys/config', queryParams);
		})
		.then((response: any) => {
